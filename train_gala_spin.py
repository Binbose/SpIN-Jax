...
import helper
from gala_backbone import make_gala_net
...
def create_train_state(n_dense_neurons, n_eigenfuncs, batch_size, D_min, D_max, learning_rate, decay_rate, sparsifying_K, n_space_dimension=2, init_rng=0):
    ...
    last_shape, weight_dict = init(init_rng, batch.shape)
...
class ModelTrainer:
    def __init__(self) -> None:
        ...
        self.save_dir = './results/{}_{}d_gala'.format(self.system, self.n_space_dimension)
    ...
    def start_training(self, show_progress=True, callback=None):
        ...
        j_sigma_t_bar = jax.tree_map(lambda x: jnp.zeros((self.n_eigenfuncs, self.n_eigenfuncs) + x.shape), weight_dict)
